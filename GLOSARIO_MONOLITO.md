...
| 33             | **APP_VERSION**                              | `"1.7.12"` — se muestra en loading screens y footer del login                                                                                                                                                                                                                                                                                                                                                                         |
...
| 211–328        | **Fetch & Auth**                             | `_fetchWithTimeout` (keepalive support, sanitiza body), `_refreshPromise` / `_refreshCooldownUntil` (dedup+cooldown 30s), `_refreshToken` (on fail: clearSession + SIGNED_OUT), `_getValidSession` (refresh si <60s restantes)                                                                                                                                                                                                                                            |
...
| 15364–15545    | **IntensityPickerModal**                     | Modal de selección de intensidad (IRM 65-95%). Grid preview: 8 columnas INTENSIDADES + columna Σ. Lee `liftplan_tablas` de localStorage para mostrar distribución de reps (PREVIEW_REPS=14) según la tabla seleccionada. Keyboard: digits directos, arrow nav, wheel capture |
...
**Props:** `value`, `onSelect`, `onClose`, `tabla` (1..5, default 1)
**State:** `activeValue`, `typedBuffer` (digit accumulator, reset 900ms)
**Refs:** `listRef`, `typedBufferResetRef`
**Computed:** `tablas` (useMemo, reads `liftplan_tablas` localStorage → `TABLA_DEFAULT` fallback), `previewGrid` (useMemo, maps IRM_VALUES × INTENSIDADES → reps cells + Σ total for `PREVIEW_REPS=14`)
**Helpers:** `scrollToIntensity`, `commitSelection`
**Layout:** Grid `56px repeat(8,1fr) 40px` — [IRM%, 50,60,70,75,80,85,90,95, Σ]. Sticky header. Rows = IRM_VALUES (65..95). `·` for zeros (opacity 0.3). Yellow Σ when total ≠ PREVIEW_REPS.
**Keyboard:** Enter, ArrowDown/Up, digits 0-9, Backspace
**Events:** Wheel capture (preventDefault)
**Constant:** `PREVIEW_REPS = 14` (defined above the component)
...
| ✅ v1.7.7    | Cronómetro: IntensityMiniCard y IntensitySteps definidos en TabataExerciseInfo.tsx pero nunca renderizados — mini-cards de % previo/siguiente invisibles | Agregado bloque JSX que renderiza `IntensityMiniCard` (prev done / next upcoming) e `IntensitySteps` dots dentro del HUD panel, después del bloque de notas |
| ✅ v1.7.7    | Cronómetro: botones de navegación (Prev/Pause/Check/Next) ocultos durante fase "countdown" activa — condición `(phase !== "countdown" || !isRunning)` ocultaba el div de controles | Removida la condición wrapper en TabataTimer.tsx; controles ahora siempre visibles cuando el timer está activo |
| ✅ v1.7.7    | Cronómetro: círculo de progreso SVG daba 2 vueltas en countdown inicial de 10s — `totalPhaseTime` se computaba desde `config.countdownTime` (5s) pero `start()` usaba 10s para primer countdown, causando progress=(5-timeLeft)/5 con timeLeft=10→0, wrapping double | Nuevo campo `phaseStartTime` en `TimerState` (types.ts) trackeando el timeLeft inicial de cada fase. Todas las transiciones del reducer (TICK, NEXT/PREV_EXERCISE, NEXT/PREV_BLOCK, SKIP_FORWARD, SKIP_PHASE) setean `phaseStartTime`. `totalPhaseTime` ahora usa `state.phaseStartTime` en vez de recomputar desde config. `RESTART_PHASE` simplificado a usar `state.phaseStartTime` |
| ✅ v1.7.11   | Auth: refresh token expirado causaba 400 en loop — `_refreshToken` seteaba `_session = null` pero no limpiaba localStorage, así que en cada reload se volvía a intentar con el token inválido | `_refreshToken` ahora llama `clearSession()` y `_emitAuth("SIGNED_OUT", null)` en los 3 paths de fallo (HTTP error, no access_token, catch). Solo borra `sb_session`, no afecta plantillas/mesos/normativos |
