...
| 33             | **APP_VERSION**                              | `"1.5.1"` — se muestra en loading screens y footer del login                                                                                                                                                                                                                                                                                                                                    |
...
| Archivo                         | Descripción                                                                                                                                                                                                                                                                                                                             |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `types.ts`                      | Interfaces: `TabataConfig`, `TabataExercise` (+ `baseId`, `baseName`, `intensityLabel`, `intensityIndex`, `totalIntensities`), `TimerPhase` (+ `intensityRest`), `TimerState`, `TimerAction` (START/TICK/PAUSE/RESUME/RESET/NEXT_EXERCISE/PREV_EXERCISE/SKIP_FORWARD/SKIP_PHASE/RESTART_PHASE). TICK/SKIP_PHASE ahora incluyen `nextExerciseSameGroup`, `nextExerciseRounds`, `countdownTime` |
| `constants.ts`                  | `DEFAULT_CONFIG` (work:120, rest:90, rounds:8, countdown:5), `STORAGE_KEY`, `TUTORIAL_SEEN_KEY`, `PHASE_COLORS` (7 fases incl. intensityRest: gold-dark), `PHASE_LABELS` (intensityRest: "CARGÁ LA BARRA"), `CAT_COLORS` (5 categorías), min/max/step                                                                                                                                    |
| `hooks/useTabataTimer.ts`       | `useReducer` state machine: idle→countdown→work→rest→(repeat)→exerciseComplete→finished. **New:** work último round → `intensityRest` si `nextExerciseSameGroup` (auto-rest entre intensidades, auto-avanza al terminar sin pausar). `exerciseRoundsRef` (useRef) + `exerciseBaseIdsRef` (useRef para pre-computar baseId array). Callbacks: start, pause, resume, reset, nextExercise, skipForward, prevExercise, skipPhase, restartPhase. `totalPhaseTime`: intensityRest → restTime                                                                                                               |
...
| `TabataTimer.tsx` (orquestador) | Props: `exercises?`, `turnoInfo?`, `onBack`. States: config, disabledIds (Set), showListModal, showTutorial, showExitModal, dontShowAgain. `groupInfo` useMemo: computa prevGroupExercise/nextGroupExercise/groupIndex/totalGroups por baseId. `exerciseGroups` useMemo: agrupa exercises[] por baseId para idle list (type Group = {baseId, baseName, exercises[]}). Sound: intensityRest dispara restStart. Idle list: grupos con header dorado + sub-items con intensityLabel + badge formato `gIdx.subIdx`. **Exercise toggle** on/off. **List modal** bottom-sheet. **Tutorial modal** (localStorage). **Exit modal** confirmación |
