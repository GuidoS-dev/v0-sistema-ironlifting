...
| Archivo                         | Descripción                                                                                                                                                                                                                                                                                                                             |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `types.ts`                      | Interfaces: `TabataConfig`, `TabataExercise`, `TimerPhase`, `TimerState`, `TimerAction` (START/TICK/PAUSE/RESUME/RESET/NEXT_EXERCISE/PREV_EXERCISE/SKIP_FORWARD/SKIP_PHASE/RESTART_PHASE). NEXT/PREV/SKIP_FORWARD usan `exerciseRounds: number[]` en vez de `totalRounds` suelto |
...
| `hooks/useTabataTimer.ts`       | `useReducer` state machine: idle→countdown→work→rest→(repeat)→exerciseComplete→finished. `exerciseRoundsRef` (useRef) almacena array de series de todos los ejercicios para evitar stale closures. Callbacks: start, pause, resume, reset, nextExercise, skipForward (series then exercise), prevExercise, skipPhase, restartPhase                                                                                                                |
